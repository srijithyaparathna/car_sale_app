<!DOCTYPE html>
<!--[if IE 8 ]><html class="ie" xmlns="http://www.w3.org/1999/xhtml" xml:lang="en-US" lang="en-US"> <![endif]-->
<!--[if (gte IE 9)|!(IE)]><!-->
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en-US" lang="en-US">
<!--<![endif]-->

<head>
    <!-- Basic Page Needs -->
    <meta charset="utf-8">
    <!--[if IE]><meta http-equiv='X-UA-Compatible' content='IE=edge,chrome=1'><![endif]-->
    <title>R&R Auto World</title>

    <meta name="author" content="themesflat.com">

    <!-- Mobile Specific Metas -->
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">

    <!-- Theme Style -->
    <link rel="stylesheet" type="text/css" href="<?= base_url('assets/css/style.css') ?>">
    <link rel="stylesheet" type="text/css" href="<?= base_url('assets/css/magnific-popup.min.css') ?>">
    <link rel="stylesheet" type="text/css" href="<?= base_url('assets/css/carsearch.css') ?>">   

    <!-- Favicon and Touch Icons  -->
    <link rel="shortcut icon" href="<?= base_url('assets/images/logo/favicon.png') ?>">
    <link rel="stylesheet" type="text/css" href="<?= base_url('assets/css/nice-select.css') ?>">
    <link rel="apple-touch-icon-precomposed" href="<?= base_url('assets/images/logo/logo.png') ?>">

    <!--Whats app-->
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css">



</head>

<body class="body counter-scroll">

<!-- preload -->
<div class="preload preload-container">
    <div class="middle">
    </div>
</div>
<!-- /preload -->

<!-- /#page -->
<div id="wrapper">
    <div id="page" class="clearfix">

        <!-- Main Header -->

        <header id="header3" class="main-header header header-fixed ">
            <!-- Header Lower -->
            <div class="top-bar">
                <div class="themesflat-container">
                    <div class="row">
                        <div class="col-md-9">
                            <ul class="list-infor-topbar">
                                <li>
                                    <a href="tel:+44 7877 354243">
                                        <i class="icon-Group-11"></i>
                                        <p>+44 7903 058693</p>
                                    </a>
                                </li>
                                <li>
                                    <a href="mailto:info@randrautoworld.co.uk">
                                        <i class="icon-Group3"></i>
                                        <p>info@randrautoworld.co.uk</p>
                                    </a>
                                </li>
                            </ul>
                        </div>
                        <div class="col-md-3">
                            <ul class="icon-topbar">
                                <li>
                                    <a href="#"><i class="icon-6"></i></a>
                                </li>
                                <li>
                                    <a href="#"><i class="icon-4"></i></a>
                                </li>
                                <li>
                                    <a href="#"><i class="icon-5"></i></a>
                                </li>
                                <li>
                                    <a href="#"><i class="icon-7"></i></a>
                                </li>

                            </ul>

                        </div>
                    </div>
                </div>
            </div>
            <div class="header-lower">
                <div class="themesflat-container">
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="header-style2 flex justify-space align-center">
                                <!-- Logo Box -->
                                <div class="logo-box flex">
                                    <div class="logo" style="padding: 5px 0;">
                                        <a href="<?= base_url('/'); ?>">
                                            <!--    <img src="assets/images/logo/logo2@.png" alt="Logo">    -->
                                            <!--<svg xmlns="http://www.w3.org/2000/svg"
                                                xmlns:xlink="http://www.w3.org/1999/xlink" id="Group_756"
                                                data-name="Group 756" width="100" height="100"
                                                viewBox="0 0 491.165 365.426">
                                                <defs>
                                                    <clipPath id="clip-path">
                                                        <rect id="Rectangle_761" data-name="Rectangle 761"
                                                            width="491.165" height="365.426" fill="none" />
                                                    </clipPath>
                                                </defs>
                                                <rect id="Rectangle_759" data-name="Rectangle 759" width="18.081"
                                                    height="42.396" transform="translate(255.584 42.189)"
                                                    fill="#231f20" />
                                                <rect id="Rectangle_760" data-name="Rectangle 760" width="53.65"
                                                    height="201.32" transform="translate(219.767 105.759)"
                                                    fill="#c00" />
                                                <g id="Group_755" data-name="Group 755">
                                                    <g id="Group_754" data-name="Group 754"
                                                        clip-path="url(#clip-path)">
                                                        <path id="Path_29" data-name="Path 29"
                                                            d="M419.715,184.312c46.875-13.2,65.078-54.155,65.078-92.383C484.793,42.779,455.213,0,386.038,0H0V53.7H176.938c25.155,0,38.456,13.495,41.875,30.966H273.63A100.9,100.9,0,0,0,266.954,53.7h120.9c29.581,0,42.779,18.657,42.779,40.5s-12.288,42.778-42.779,42.778H326.848v51.425h34.615l70.085,118.779h59.617Z"
                                                            fill="#231f20" />
                                                        <path id="Path_30" data-name="Path 30"
                                                            d="M219.767,217.893l-.027-.046-19.5-33.535.068-.02-27.483-47.457h-.333c-1.335.088-2.7.148-4.108.148H48.695c-33.677,0-48.24,14.108-48.24,48.695V307.187h53.7V196.145c0-5.461,2.731-7.737,7.737-7.737h80.1l70.085,118.779h7.694Z"
                                                            fill="#c00" />
                                                        <path id="Path_31" data-name="Path 31"
                                                            d="M172.786,136.836l-30.8,51.572,70.085,118.779h61.558Z"
                                                            fill="#231f20" />
                                                        <path id="Path_32" data-name="Path 32"
                                                            d="M22.239,365.024,12.7,348.655H6.387v16.369H1.16V324.993H14.083a17.173,17.173,0,0,1,7.668,1.55,10.807,10.807,0,0,1,4.681,4.193,12.422,12.422,0,0,1-.833,13.325,11.655,11.655,0,0,1-7.151,4.192L28.5,365.024ZM6.387,344.463h7.7q4.25,0,6.376-2.1a7.511,7.511,0,0,0,2.124-5.6,7.163,7.163,0,0,0-2.1-5.513q-2.1-1.951-6.4-1.953h-7.7Z"
                                                            fill="#231f20" />
                                                        <path id="Path_33" data-name="Path 33"
                                                            d="M67.554,365.024H62.327l-21.02-31.876v31.876H36.08V324.935h5.227l21.02,31.818V324.935h5.227Z"
                                                            fill="#231f20" />
                                                        <path id="Path_34" data-name="Path 34"
                                                            d="M97.534,365.024,88,348.655H81.682v16.369H76.455V324.993H89.378a17.173,17.173,0,0,1,7.668,1.55,10.807,10.807,0,0,1,4.681,4.193,11.6,11.6,0,0,1-7.984,17.517l10.051,16.771ZM81.682,344.463h7.7q4.25,0,6.376-2.1a7.511,7.511,0,0,0,2.124-5.6,7.163,7.163,0,0,0-2.1-5.513q-2.1-1.951-6.4-1.953h-7.7Z"
                                                            fill="#231f20" />
                                                        <path id="Path_35" data-name="Path 35"
                                                            d="M150.372,356.122h-17.46l-3.216,8.9h-5.514l14.473-39.8h6.031l14.416,39.8h-5.514Zm-1.493-4.25-7.237-20.217L134.4,351.872Z"
                                                            fill="#231f20" />
                                                        <path id="Path_36" data-name="Path 36"
                                                            d="M170.531,324.993v25.328q0,5.341,2.613,7.926a11.423,11.423,0,0,0,14.474,0q2.611-2.585,2.613-7.926V324.993h5.227v25.271a16.353,16.353,0,0,1-2.01,8.414,12.625,12.625,0,0,1-5.428,5.083,18.487,18.487,0,0,1-15.335,0,12.44,12.44,0,0,1-5.4-5.083,16.521,16.521,0,0,1-1.982-8.414V324.993Z"
                                                            fill="#231f20" />
                                                        <path id="Path_37" data-name="Path 37"
                                                            d="M228.827,324.993v4.25H217.915v35.781h-5.227V329.243h-10.97v-4.25Z"
                                                            fill="#231f20" />
                                                        <path id="Path_38" data-name="Path 38"
                                                            d="M243.242,362.812a19.108,19.108,0,0,1-7.265-7.294,22.127,22.127,0,0,1,0-21.078,19.122,19.122,0,0,1,7.265-7.294,21.188,21.188,0,0,1,20.39,0,18.982,18.982,0,0,1,7.236,7.266,22.455,22.455,0,0,1,0,21.135,18.973,18.973,0,0,1-7.236,7.265,21.188,21.188,0,0,1-20.39,0m17.719-3.876a13.744,13.744,0,0,0,5.284-5.572,19.294,19.294,0,0,0,0-16.8,13.87,13.87,0,0,0-5.256-5.544,15.7,15.7,0,0,0-15.162,0,13.855,13.855,0,0,0-5.255,5.544,19.285,19.285,0,0,0,0,16.8,13.735,13.735,0,0,0,5.283,5.572,15.582,15.582,0,0,0,15.106,0"
                                                            fill="#231f20" />
                                                        <path id="Path_39" data-name="Path 39"
                                                            d="M346.049,324.993l-11.6,40.031h-5.859l-9.3-32.22-9.649,32.22-5.8.058-11.2-40.089h5.571l8.73,33.944,9.649-33.944h5.859l9.189,33.829,8.787-33.829Z"
                                                            fill="#231f20" />
                                                        <path id="Path_40" data-name="Path 40"
                                                            d="M359.774,362.812a19.108,19.108,0,0,1-7.265-7.294,22.128,22.128,0,0,1,0-21.078,19.122,19.122,0,0,1,7.265-7.294,21.188,21.188,0,0,1,20.39,0,18.982,18.982,0,0,1,7.236,7.266,22.455,22.455,0,0,1,0,21.135,18.972,18.972,0,0,1-7.236,7.265,21.188,21.188,0,0,1-20.39,0m17.719-3.876a13.735,13.735,0,0,0,5.283-5.572,19.285,19.285,0,0,0,0-16.8,13.861,13.861,0,0,0-5.255-5.544,15.7,15.7,0,0,0-15.162,0,13.854,13.854,0,0,0-5.255,5.544,19.285,19.285,0,0,0,0,16.8,13.735,13.735,0,0,0,5.283,5.572,15.582,15.582,0,0,0,15.106,0"
                                                            fill="#231f20" />
                                                        <path id="Path_41" data-name="Path 41"
                                                            d="M418.013,365.024l-9.534-16.369h-6.318v16.369h-5.227V324.993h12.923a17.173,17.173,0,0,1,7.668,1.55,10.813,10.813,0,0,1,4.681,4.193,11.605,11.605,0,0,1-7.984,17.517l10.051,16.771Zm-15.852-20.561h7.7q4.25,0,6.375-2.1a7.511,7.511,0,0,0,2.125-5.6,7.163,7.163,0,0,0-2.1-5.513q-2.1-1.951-6.4-1.953h-7.7Z"
                                                            fill="#231f20" />
                                                        <path id="Path_42" data-name="Path 42"
                                                            d="M437.08,360.774h14.014v4.25H431.853V324.992h5.227Z"
                                                            fill="#231f20" />
                                                        <path id="Path_43" data-name="Path 43"
                                                            d="M480.471,327.433a17.166,17.166,0,0,1,7.351,6.979,23.543,23.543,0,0,1,0,21.337,16.855,16.855,0,0,1-7.351,6.892,25.217,25.217,0,0,1-11.343,2.383H456.664V324.993h12.464a24.721,24.721,0,0,1,11.343,2.44m.488,29.177q4.077-4.105,4.078-11.515,0-7.465-4.106-11.66t-11.8-4.192h-7.237v31.473h7.237q7.754,0,11.831-4.106"
                                                            fill="#231f20" />
                                                    </g>
                                                </g>
                                            </svg>-->
                                            <svg id="Group_133" data-name="Group 133" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="250" height="auto" viewBox="0 0 742.172 248.684">
                                                <defs>
                                                    <clipPath id="clip-path">
                                                        <rect id="Rectangle_171" data-name="Rectangle 171" width="742.172" height="248.684" fill="none"/>
                                                    </clipPath>
                                                </defs>
                                                <g id="Group_132" data-name="Group 132" clip-path="url(#clip-path)">
                                                    <path id="Path_430" data-name="Path 430" d="M248.684,124.342A124.342,124.342,0,1,1,124.342,0,124.342,124.342,0,0,1,248.684,124.342" fill="#54080b"/>
                                                    <path id="Path_431" data-name="Path 431" d="M85.859,66.105c4.62-5.953,10.033-11.173,17.347-13.482a70.772,70.772,0,0,1,14.9-2.79,151.532,151.532,0,0,1,34.061,1.037c11.955,1.63,23.211,5.672,34.235,10.352,14.841,6.3,17.133,25.47,4.324,35.23a61.485,61.485,0,0,1-17.606,8.989c-6.314,2.166-12.627,4.338-18.895,6.633-1.985.728-2.082,1.565-.694,3.127q6.675,7.512,13.389,14.987c10.968,12.2,22.15,24.222,32.867,36.642,10.285,11.918,15.96,25.757,16.449,41.271A124.3,124.3,0,1,0,7.559,167.133q39.129-50.53,78.3-101.028" fill="#d7001c"/>
                                                    <path id="Path_432" data-name="Path 432" d="M193.906,215.356c1.058-14.56-4.147-26.705-13.741-37.288q-19.368-21.363-38.709-42.749a104.735,104.735,0,0,1-8.1-9.71c-8.054-11.466-5.524-23.5,6.292-31.04,6.053-3.861,12.857-5.869,19.583-8.1,3.864-1.279,7.7-2.661,11.5-4.11a1.7,1.7,0,0,0,.162-.089c1.585-1.859-1.057-3.567-1.057-3.567l0,0a98.752,98.752,0,0,0-34.206-6.88c-.435-.01-.87-.035-1.3-.058v0l-7.94.146c-3.47.162-6.943.3-10.41.493a13.575,13.575,0,0,0-10.306,5.067Q97,88.291,88.5,99.242q-31.6,40.71-63.145,81.464-3.439,4.442-6.871,8.889a124.367,124.367,0,0,0,174.578,38.379q.393-6.311.841-12.618" fill="#d7001c"/>
                                                    <path id="Path_433" data-name="Path 433" d="M167.06,115.46q-6.717-7.473-13.389-14.986c-1.388-1.563-1.291-2.4.694-3.127,6.268-2.3,12.581-4.468,18.895-6.634a61.484,61.484,0,0,0,17.606-8.989c12.809-9.76,10.517-28.93-4.324-35.23-11.024-4.68-22.28-8.722-34.235-10.351A151.532,151.532,0,0,0,118.246,35.1a70.772,70.772,0,0,0-14.9,2.79C96.028,40.205,90.615,45.424,86,51.377Q45.109,104.064,4.289,156.8a123.571,123.571,0,0,0,9.443,24.391q5.883-7.606,11.761-15.217,31.542-40.755,63.145-81.463,8.5-10.95,17.171-21.767a13.581,13.581,0,0,1,10.306-5.067c3.467-.195,6.94-.33,10.41-.493l7.94-.145v0c.435.023.87.048,1.305.058a98.761,98.761,0,0,1,34.207,6.879l0,0s2.642,1.707,1.057,3.566a1.514,1.514,0,0,1-.162.09c-3.8,1.448-7.636,2.83-11.5,4.109-6.726,2.227-13.53,4.235-19.583,8.1-11.816,7.536-14.346,19.574-6.292,31.04a104.59,104.59,0,0,0,8.1,9.709Q160.9,142.009,180.3,163.34c9.6,10.583,14.8,22.728,13.741,37.288-.674,9.277-1.216,18.564-1.745,27.852a124.95,124.95,0,0,0,23.643-20.057c.189-3.593.35-7.186.455-10.781.505-17.257-5.247-32.538-16.467-45.54-10.717-12.419-21.9-24.438-32.867-36.642" fill="#fff"/>
                                                    <path id="Path_434" data-name="Path 434" d="M351.26,47.808q28.5,0,40.47,7.028t11.968,25.5a25.024,25.024,0,0,1-6.929,17.683q-7.716,8.208-21.731,9.63,13.542,0,20.157,5.512t6.613,15.589v23.621H368.9V132.055q0-6.768-5.669-9.369t-16.849-2.6H305.437v32.281H273.628V47.808Zm-4.881,49.761q13.541,0,18.66-2.613t5.118-10.693q0-7.444-2.992-9.822-4.41-3.485-18.9-3.484H305.437V97.569Z" fill="#1a1818"/>
                                                    <path id="Path_435" data-name="Path 435" d="M555.182,126.544l23.149,13.07-16.377,13.07-18.74-10.393q-6.93,6.613-20.628,9.133-9.607,1.731-25.668,1.732H479.282q-23.148,0-35.116-5.2Q425.9,139.929,425.9,118.67q0-11.965,5.354-17.794,6.141-6.613,21.889-8.661a41.187,41.187,0,0,1-7.244-7.4,19.12,19.12,0,0,1-3.149-10.866q0-15.43,14.172-21.1Q467.628,48.6,493.139,48.6q27.241,0,37.636,4.409,13.7,5.827,13.7,22.991v6.771H515.5q0-11.179-2.678-13.543-3.467-3.147-18.581-3.149-11.183,0-14.33.787-6.459,1.577-6.456,7.244,0,5.042,2.2,7.559,1.731,1.889,8.818,5.826l45.509,25.353a16.058,16.058,0,0,0,1.26-6.3,13.632,13.632,0,0,0-1.26-6.142h27.4q.787,4.412,1.024,7.244t.236,4.567a28.72,28.72,0,0,1-1.417,9.133q-.473,1.417-2.048,5.2m-58.578,8.5a101.838,101.838,0,0,0,14.645-.787,24.388,24.388,0,0,0,9.92-4.094L472.2,103.4q-10.08,1.575-12.834,4.172t-2.756,9.685q0,11.811,7.716,15.589,4.566,2.208,17.165,2.205Z" fill="#1a1818"/>
                                                    <path id="Path_436" data-name="Path 436" d="M671.867,47.808q28.5,0,40.47,7.028t11.967,25.5a25.023,25.023,0,0,1-6.928,17.683q-7.717,8.208-21.731,9.63,13.542,0,20.156,5.512t6.614,15.589v23.621H689.5V132.055q0-6.768-5.669-9.369t-16.85-2.6H626.043v32.281H594.234V47.808Zm-4.882,49.761q13.541,0,18.66-2.613t5.118-10.693q0-7.444-2.992-9.822-4.41-3.485-18.9-3.484H626.043V97.569Z" fill="#1a1818"/>
                                                    <path id="Path_437" data-name="Path 437" d="M303.64,168.488l16.654,32.145H310.079l-3.1-5.954H286.745l-2.953,5.954H273.286l16.363-32.145Zm-13.8,19.268h14.088l-7.117-13.41Z" fill="#d7001b"/>
                                                    <path id="Path_438" data-name="Path 438" d="M365.364,168.585v21.2a8.891,8.891,0,0,1-4.962,8.448q-4.964,2.64-15.08,2.639-10.893,0-15.347-2.131-5.471-2.614-5.47-9.73v-20.43h10.021v20.43q0,3.05,2.082,4.018,1.742.823,6.68.823h3.2a28.6,28.6,0,0,0,6.584-.436q2.274-.726,2.275-3.631v-21.2Z" fill="#d7001b"/>
                                                    <path id="Path_439" data-name="Path 439" d="M407.868,168.488v7.165H393.684v24.98H383.565v-24.98H369.623v-7.165Z" fill="#d7001b"/>
                                                    <path id="Path_440" data-name="Path 440" d="M453.036,178.185V190.6q0,4.945-4.938,7.757a19.872,19.872,0,0,1-9.924,2.376H434.2q-5.375,0-7.044-.016t-2.929-.081a16.073,16.073,0,0,1-9.295-2.614,8.6,8.6,0,0,1-4.163-7.455V178.17q0-4.21,4.357-7.116a15.19,15.19,0,0,1,8.569-2.566h14.91a18.971,18.971,0,0,1,9.876,2.424q4.55,2.717,4.551,7.273m-10.167,9.394v-5.844a6.977,6.977,0,0,0-1.694-5.07q-1.889-1.98-5.954-1.98h-8.037a6.087,6.087,0,0,0-4.817,1.787,7.349,7.349,0,0,0-1.573,5.022v6.085a7.076,7.076,0,0,0,1.549,4.877,6.166,6.166,0,0,0,4.841,1.739h8.037q4.114,0,5.881-1.642a6.472,6.472,0,0,0,1.767-4.974" fill="#d7001b"/>
                                                    <path id="Path_441" data-name="Path 441" d="M532.091,168.488H542.4l-13.362,32.145h-11.86L507.5,177.978l-7.5,22.656H488.327l-12.973-32.145h10.214l8.617,23.431,7.939-23.431H513.4l9.828,23.431Z" fill="#d7001b"/>
                                                    <path id="Path_442" data-name="Path 442" d="M586.7,178.185V190.6q0,4.945-4.938,7.757a19.872,19.872,0,0,1-9.924,2.376h-3.97q-5.375,0-7.044-.016t-2.929-.081a16.073,16.073,0,0,1-9.295-2.614,8.6,8.6,0,0,1-4.163-7.455V178.17q0-4.21,4.357-7.116a15.19,15.19,0,0,1,8.569-2.566h14.91a18.971,18.971,0,0,1,9.876,2.424q4.55,2.717,4.551,7.273m-10.167,9.394v-5.844a6.977,6.977,0,0,0-1.694-5.07q-1.889-1.98-5.954-1.98h-8.037a6.087,6.087,0,0,0-4.817,1.787,7.349,7.349,0,0,0-1.573,5.022v6.085a7.076,7.076,0,0,0,1.549,4.877,6.166,6.166,0,0,0,4.841,1.739h8.037q4.115,0,5.881-1.642a6.472,6.472,0,0,0,1.767-4.974" fill="#d7001b"/>
                                                    <path id="Path_443" data-name="Path 443" d="M616.907,168.488q8.762,0,12.441,2.16t3.68,7.839a7.7,7.7,0,0,1-2.13,5.437,10.222,10.222,0,0,1-6.681,2.961q4.163,0,6.2,1.694a5.893,5.893,0,0,1,2.033,4.792v7.262H622.329v-6.245a2.9,2.9,0,0,0-1.743-2.88,12.949,12.949,0,0,0-5.18-.8H602.819v9.924H593.04V168.488Zm-1.5,15.3q4.163,0,5.737-.8t1.574-3.288q0-2.288-.92-3.019-1.357-1.071-5.81-1.071H602.819v8.181Z" fill="#d7001b"/>
                                                    <path id="Path_444" data-name="Path 444" d="M672.772,193.469v7.165H640.385V168.489H650.5v24.98Z" fill="#d7001b"/>
                                                    <path id="Path_445" data-name="Path 445" d="M720.313,180.415l.1,5.382q0,7.128-2.9,10.424-3.873,4.412-14.232,4.412H677.566V168.488h24.108q9.681,0,14.16,2.813t4.479,9.114m-20.091,13.828q6.293,0,8.375-2.33,1.5-1.7,1.5-6.164v-4.8a4.452,4.452,0,0,0-1.743-3.932q-2.131-1.5-7.552-1.5H687.538v18.735Z" fill="#d7001b"/>
                                                    <path id="Path_446" data-name="Path 446" d="M730.878,38.486v1.021h-2.62V48.1H727V39.507h-2.634V38.486Z" fill="#1a1818"/>
                                                    <path id="Path_447" data-name="Path 447" d="M742.172,38.555V48.1h-1.255V40.982L737.745,48.1h-.883l-3.185-7.129V48.1h-1.255V38.555h1.352l3.53,7.888,3.53-7.888Z" fill="#1a1818"/>
                                                </g>
                                            </svg>

                                        </a>
                                    </div>
                                </div>
                                <div class="nav-outer flex align-center">
                                    <!-- Main Menu -->
                                    <nav class="main-menu show navbar-expand-md">
                                        <div class="navbar-collapse collapse clearfix" id="navbarSupportedContent">
                                            <ul class="navigation clearfix">
                                                <li class="current">
                                                    <a href="<?= base_url('/'); ?>">Home</a>
                                                </li>
                                                <li>
                                                    <a href="<?= base_url('/cars?main-vehicle-type=Passenger'); ?>">Cars</a>
                                                </li>
                                                <li>
                                                    <a href="<?= base_url('/cars?main-vehicle-type=Commercial'); ?>">Commercial Vehicles</a>
                                                </li>
                                                <!--<li>
                                                        <a href="<?= base_url('/cars?main-vehicle-type=Machinery'); ?>">Machinery</a>
                                                    </li>
                                                    <li>
                                                        <a href="<?= base_url('/cars?main-vehicle-type=Motorcycle'); ?>">Motorcycles</a>
                                                    </li>-->
                                                <li>
                                                    <a href="<?= base_url('/about-us'); ?>">About us </a>
                                                </li>
                                                <li>
                                                    <a href="<?= base_url('/contact-us'); ?>">Contact us </a>
                                                </li>



                                            </ul>
                                        </div>
                                    </nav>
                                    <!-- Main Menu End-->
                                </div>
                                <!-- <div class="header-account flex align-center">
                                <div class="register ml--18">
                                    <div class="flex align-center">
                                        <a data-bs-toggle="modal" href="#exampleModalToggle" role="button">Register</a>
                                        <a data-bs-toggle="modal" href="#exampleModalToggle2" role="button">Login</a>
                                    </div>
                                </div>
                                <div class="flat-bt-top sc-btn-top ml--20 ">
                                    <a class="btn-icon-list" href="car-list.html">
                                        <span>Listing Yours</span>
                                        <i class="icon-add-button-1"></i>
                                    </a>
                                </div>
                            </div> -->
                                <div class="mobile-nav-toggler mobile-button">
                                    <span></span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <!-- End Header Lower -->
            <!-- Mobile Menu  -->
            <div class="close-btn"><span class="icon flaticon-cancel-1"></span></div>
            <div class="mobile-menu">
                <div class="menu-backdrop"></div>
                <nav class="menu-box">
                    <div class="nav-logo">
                        <a href="<?= base_url('/'); ?>">
                            <!--                            <img src="assets/images/logo/logo2@.png" alt="Logo Motorx">-->
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                 id="Group_756" data-name="Group 756" width="100" height="100"
                                 viewBox="0 0 491.165 365.426">
                                <defs>
                                    <clipPath id="clip-path">
                                        <rect id="Rectangle_761" data-name="Rectangle 761" width="491.165"
                                              height="365.426" fill="none" />
                                    </clipPath>
                                </defs>
                                <rect id="Rectangle_759" data-name="Rectangle 759" width="18.081" height="42.396"
                                      transform="translate(255.584 42.189)" fill="#231f20" />
                                <rect id="Rectangle_760" data-name="Rectangle 760" width="53.65" height="201.32"
                                      transform="translate(219.767 105.759)" fill="#c00" />
                                <g id="Group_755" data-name="Group 755">
                                    <g id="Group_754" data-name="Group 754" clip-path="url(#clip-path)">
                                        <path id="Path_29" data-name="Path 29"
                                              d="M419.715,184.312c46.875-13.2,65.078-54.155,65.078-92.383C484.793,42.779,455.213,0,386.038,0H0V53.7H176.938c25.155,0,38.456,13.495,41.875,30.966H273.63A100.9,100.9,0,0,0,266.954,53.7h120.9c29.581,0,42.779,18.657,42.779,40.5s-12.288,42.778-42.779,42.778H326.848v51.425h34.615l70.085,118.779h59.617Z"
                                              fill="#231f20" />
                                        <path id="Path_30" data-name="Path 30"
                                              d="M219.767,217.893l-.027-.046-19.5-33.535.068-.02-27.483-47.457h-.333c-1.335.088-2.7.148-4.108.148H48.695c-33.677,0-48.24,14.108-48.24,48.695V307.187h53.7V196.145c0-5.461,2.731-7.737,7.737-7.737h80.1l70.085,118.779h7.694Z"
                                              fill="#c00" />
                                        <path id="Path_31" data-name="Path 31"
                                              d="M172.786,136.836l-30.8,51.572,70.085,118.779h61.558Z"
                                              fill="#231f20" />
                                        <path id="Path_32" data-name="Path 32"
                                              d="M22.239,365.024,12.7,348.655H6.387v16.369H1.16V324.993H14.083a17.173,17.173,0,0,1,7.668,1.55,10.807,10.807,0,0,1,4.681,4.193,12.422,12.422,0,0,1-.833,13.325,11.655,11.655,0,0,1-7.151,4.192L28.5,365.024ZM6.387,344.463h7.7q4.25,0,6.376-2.1a7.511,7.511,0,0,0,2.124-5.6,7.163,7.163,0,0,0-2.1-5.513q-2.1-1.951-6.4-1.953h-7.7Z"
                                              fill="#231f20" />
                                        <path id="Path_33" data-name="Path 33"
                                              d="M67.554,365.024H62.327l-21.02-31.876v31.876H36.08V324.935h5.227l21.02,31.818V324.935h5.227Z"
                                              fill="#231f20" />
                                        <path id="Path_34" data-name="Path 34"
                                              d="M97.534,365.024,88,348.655H81.682v16.369H76.455V324.993H89.378a17.173,17.173,0,0,1,7.668,1.55,10.807,10.807,0,0,1,4.681,4.193,11.6,11.6,0,0,1-7.984,17.517l10.051,16.771ZM81.682,344.463h7.7q4.25,0,6.376-2.1a7.511,7.511,0,0,0,2.124-5.6,7.163,7.163,0,0,0-2.1-5.513q-2.1-1.951-6.4-1.953h-7.7Z"
                                              fill="#231f20" />
                                        <path id="Path_35" data-name="Path 35"
                                              d="M150.372,356.122h-17.46l-3.216,8.9h-5.514l14.473-39.8h6.031l14.416,39.8h-5.514Zm-1.493-4.25-7.237-20.217L134.4,351.872Z"
                                              fill="#231f20" />
                                        <path id="Path_36" data-name="Path 36"
                                              d="M170.531,324.993v25.328q0,5.341,2.613,7.926a11.423,11.423,0,0,0,14.474,0q2.611-2.585,2.613-7.926V324.993h5.227v25.271a16.353,16.353,0,0,1-2.01,8.414,12.625,12.625,0,0,1-5.428,5.083,18.487,18.487,0,0,1-15.335,0,12.44,12.44,0,0,1-5.4-5.083,16.521,16.521,0,0,1-1.982-8.414V324.993Z"
                                              fill="#231f20" />
                                        <path id="Path_37" data-name="Path 37"
                                              d="M228.827,324.993v4.25H217.915v35.781h-5.227V329.243h-10.97v-4.25Z"
                                              fill="#231f20" />
                                        <path id="Path_38" data-name="Path 38"
                                              d="M243.242,362.812a19.108,19.108,0,0,1-7.265-7.294,22.127,22.127,0,0,1,0-21.078,19.122,19.122,0,0,1,7.265-7.294,21.188,21.188,0,0,1,20.39,0,18.982,18.982,0,0,1,7.236,7.266,22.455,22.455,0,0,1,0,21.135,18.973,18.973,0,0,1-7.236,7.265,21.188,21.188,0,0,1-20.39,0m17.719-3.876a13.744,13.744,0,0,0,5.284-5.572,19.294,19.294,0,0,0,0-16.8,13.87,13.87,0,0,0-5.256-5.544,15.7,15.7,0,0,0-15.162,0,13.855,13.855,0,0,0-5.255,5.544,19.285,19.285,0,0,0,0,16.8,13.735,13.735,0,0,0,5.283,5.572,15.582,15.582,0,0,0,15.106,0"
                                              fill="#231f20" />
                                        <path id="Path_39" data-name="Path 39"
                                              d="M346.049,324.993l-11.6,40.031h-5.859l-9.3-32.22-9.649,32.22-5.8.058-11.2-40.089h5.571l8.73,33.944,9.649-33.944h5.859l9.189,33.829,8.787-33.829Z"
                                              fill="#231f20" />
                                        <path id="Path_40" data-name="Path 40"
                                              d="M359.774,362.812a19.108,19.108,0,0,1-7.265-7.294,22.128,22.128,0,0,1,0-21.078,19.122,19.122,0,0,1,7.265-7.294,21.188,21.188,0,0,1,20.39,0,18.982,18.982,0,0,1,7.236,7.266,22.455,22.455,0,0,1,0,21.135,18.972,18.972,0,0,1-7.236,7.265,21.188,21.188,0,0,1-20.39,0m17.719-3.876a13.735,13.735,0,0,0,5.283-5.572,19.285,19.285,0,0,0,0-16.8,13.861,13.861,0,0,0-5.255-5.544,15.7,15.7,0,0,0-15.162,0,13.854,13.854,0,0,0-5.255,5.544,19.285,19.285,0,0,0,0,16.8,13.735,13.735,0,0,0,5.283,5.572,15.582,15.582,0,0,0,15.106,0"
                                              fill="#231f20" />
                                        <path id="Path_41" data-name="Path 41"
                                              d="M418.013,365.024l-9.534-16.369h-6.318v16.369h-5.227V324.993h12.923a17.173,17.173,0,0,1,7.668,1.55,10.813,10.813,0,0,1,4.681,4.193,11.605,11.605,0,0,1-7.984,17.517l10.051,16.771Zm-15.852-20.561h7.7q4.25,0,6.375-2.1a7.511,7.511,0,0,0,2.125-5.6,7.163,7.163,0,0,0-2.1-5.513q-2.1-1.951-6.4-1.953h-7.7Z"
                                              fill="#231f20" />
                                        <path id="Path_42" data-name="Path 42"
                                              d="M437.08,360.774h14.014v4.25H431.853V324.992h5.227Z" fill="#231f20" />
                                        <path id="Path_43" data-name="Path 43"
                                              d="M480.471,327.433a17.166,17.166,0,0,1,7.351,6.979,23.543,23.543,0,0,1,0,21.337,16.855,16.855,0,0,1-7.351,6.892,25.217,25.217,0,0,1-11.343,2.383H456.664V324.993h12.464a24.721,24.721,0,0,1,11.343,2.44m.488,29.177q4.077-4.105,4.078-11.515,0-7.465-4.106-11.66t-11.8-4.192h-7.237v31.473h7.237q7.754,0,11.831-4.106"
                                              fill="#231f20" />
                                    </g>
                                </g>
                            </svg>
                        </a>
                    </div>
                    <div class="bottom-canvas">
                        <div class="menu-outer">
                            <ul class="navigation clearfix">
                                <li class="current">
                                    <a href="<?= base_url('/'); ?>">Home</a>
                                </li>
                                <li>
                                    <a href="<?= base_url('/cars?main-vehicle-type=Passenger'); ?>">Cars</a>
                                </li>
                                <li>
                                    <a href="<?= base_url('/cars?main-vehicle-type=Machinery'); ?>">Machinery</a>
                                </li>
                                <li>
                                    <a href="<?= base_url('/cars?main-vehicle-type=Motorcycle'); ?>">Motorcycles</a>
                                </li>
                                <li>
                                    <a href="<?= base_url('/about-us'); ?>">About Us</a>
                                </li>
                                <li>
                                    <a href=" <?= base_url('/contact-us'); ?>">Contact Us</a>
                                </li>


                            </ul>
                        </div>

                    </div>
                </nav>
            </div>
            <!-- End Mobile Menu -->

        </header>
        <!-- End Main Header -->

        <!-- Slide -->
        <div class="widget-tf-slider">
            <div class="slider-wrap swiper-wrapper">
                <div class="tf-slide-item swiper-slide">
                    <div class="slide-item-image">
                        <img src="<?= base_url('/assets/images/slide/bg.jpg'); ?>" loading="lazy" alt="">
                        <div class="overlay"></div>
                    </div>
                    <div class="slide-item-content">
                        <div class="slide-content">
                                <span class="wow fadeInUp sub-title" data-wow-delay="100ms"
                                      data-wow-duration="2000ms">Trusted Dealer,</span>
                            <h1 class=" title-slide wow slideInUp" data-wow-delay="50ms" data-wow-duration="200ms">
                                Specialize for European & Japanese Vehicles</h1>
                            <p class="description wow fadeInUp" data-wow-delay="300ms" data-wow-duration="2000ms">
                                “Beyond your Expectation, Next Level….”</p>
                            <div class="box">
                                <!-- Button -->
                                <div class="btn-main wow fadeInUp" data-wow-delay="400ms"
                                     data-wow-duration="2000ms">
                                    <a href="<?= base_url('/cars'); ?>" class="button_main_inner ">
                                            <span>
                                                Go to listing
                                            </span>
                                    </a>
                                </div>

                            </div>
                        </div>

<div class="slide-image">
    <div class="box-car" >

        <div class="container mt-5">
            <div class="search-widget">
                <h4>Search Cars or Machinery</h4>
                <form method="get" id="search-form" action="<?= base_url('/cars') ?>">
                    <div class="row">


                    <div class="col-12 mb-3">
                         <select name="main-vehicle-type" id="main-vehicle-type" class="form-group">
                                         <option value="" disabled selected>Type of Vehicle</option>
                                         <option value="">Any</option>
                                                <option value="Passenger" <?= (isset($_GET['main-vehicle-type']) && $_GET['main-vehicle-type'] === 'Passenger') ? 'selected' : '' ?> >Passenger</option>
                                                <option value="Commercial" <?= (isset($_GET['main-vehicle-type']) && $_GET['main-vehicle-type'] === 'Commercial') ? 'selected' : '' ?> >Commercial</option>
                                                <option value="Public Transport" <?= (isset($_GET['main-vehicle-type']) && $_GET['main-vehicle-type'] === 'Public Transport') ? 'selected' : '' ?> >Public Transport</option>
                                                <option value="Machinery" <?= (isset($_GET['main-vehicle-type']) && $_GET['main-vehicle-type'] === 'Machinery') ? 'selected' : '' ?> >Machinery</option>
                                                <option value="Special Purposes" <?= (isset($_GET['main-vehicle-type']) && $_GET['main-vehicle-type'] === 'Special Purposes') ? 'selected' : '' ?> >Special Purposes</option>
                                                <option value="Motorcycle" <?= (isset($_GET['main-vehicle-type']) && $_GET['main-vehicle-type'] === 'Motorcycle') ? 'selected' : '' ?> >Motorcycle</option>
                                          
                                         </select>
                             </div>




                        <!-- Main Category (Make) Dropdown -->
                        <div class="col-md-6 mb-3">
                            <select name="main-category" id="make" class="form-group" onchange="updateModels(this.value)">
                                <option value="" disabled selected>Make</option>
                                <option value="">Any</option>
                                <?php foreach ($categories1 as $category): ?>
                                    <option value="<?= $category['id'] ?>" <?= isset($_GET['main-category']) && $_GET['main-category'] == $category['id'] ? 'selected' : '' ?>>
                                        <?= $category['category_name'] ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>

                        <!-- Subcategory (Model) Dropdown -->
                        <div class="col-md-6 mb-3">
                            <select name="sub-category" id="model" class="form-group">
                                
                                <option value="" disabled selected>Model</option>
                                <!-- Models will be populated dynamically -->
                            </select>
                        </div>

                        <!-- Year From -->
                        <div class="col-md-6 mb-3">
                            <!-- <label>Year From</label> -->
                            <input type="number" id="year-from" name="year-from" placeholder="Year From" min="1900" max="<?= date('Y') ?>" 
                                value="<?= isset($_GET['year-from']) ? htmlspecialchars($_GET['year-from']) : '' ?>" class="form-group">
                        </div>

                        <!-- Year To -->
                        <div class="col-md-6 mb-3">
                            <!-- <label>Year To</label> -->
                            <input type="number" id="year-to" name="year-to" placeholder="Year To" min="1900" max="<?= date('Y') ?>" 
                                value="<?= isset($_GET['year-to']) ? htmlspecialchars($_GET['year-to']) : '' ?>" class="form-group">
                        </div>

                        <!-- Price From -->
                        <div class="col-md-6 mb-3">
                            <!-- <label>Price From</label> -->
                            <input type="number" id="min-value" name="min-value" placeholder="Min Price" min="0" step="1" 
                                value="<?= isset($_GET['min-value']) ? htmlspecialchars($_GET['min-value']) : '' ?>" class="form-group">
                        </div>

                        <!-- Price To -->
                        <div class="col-md-6 mb-3">
                            <!-- <label>Price To</label> -->
                            <input type="number" id="max-value" name="max-value" placeholder="Max Price" min="0" step="1" 
                                value="<?= isset($_GET['max-value']) ? htmlspecialchars($_GET['max-value']) : '' ?>" class="form-group">
                        </div>

                        <!-- Search Button -->
                        <div class="col-12">
                            <button type="submit" class="btn btn-danger w-100">Search Cars</button>
                        </div>
                    </div>
                </form>
            </div>
        </div>
    </div>

    </div>
</div>

<script>
// JavaScript to dynamically update the models based on the selected main category (make)
function updateModels(makeId) {
    // Map categories and subcategories (adjust as per your backend structure)
    const categories = <?= json_encode($categories1) ?>;
    
    // Find the selected category
    const selectedCategory = categories.find(category => category.id == makeId);
    
    // Get the model dropdown
    const modelDropdown = document.getElementById('model');
    
    // Clear existing options
    modelDropdown.innerHTML = '<option value="" disabled selected>Model</option>';
    
    // Add new options
    if (selectedCategory && selectedCategory.sub_categories) {
        selectedCategory.sub_categories.forEach(subCategory => {
            const option = document.createElement('option');
            option.value = subCategory.id;
            option.textContent = subCategory.model_name;
            modelDropdown.appendChild(option);
        });
    }
}

// Trigger change event if there's already a selected main category
const mainCategory = document.getElementById('make');
if (mainCategory.value) {
    mainCategory.dispatchEvent(new Event('change'));
}
</script>



                        <div class="box">
                            <span>+44 7903 058693</span>
                            <span>ruchini.shell@gmail.com</span>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        
        <!-- Slide -->

        <!-- Slide search car -->
        <div class="widget-search-car">
            <div class="themesflat-container">
                <div class="search-form-widget">
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="home-tab" data-bs-toggle="tab"
                                    data-bs-target="#home" type="button" role="tab" aria-controls="home"
                                    aria-selected="true">All Cars</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="profile-tab" data-bs-toggle="tab" data-bs-target="#profile"
                                    type="button" role="tab" aria-controls="profile" aria-selected="false">Used
                                car</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="contact-tab" data-bs-toggle="tab" data-bs-target="#contact"
                                    type="button" role="tab" aria-controls="contact" aria-selected="false">New
                                Cars</button>
                        </li>
                    </ul>
                    <div class="tab-content" id="myTabContent">
                        <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">


                            <form method="get" id="search-forms" action="<?= base_url('/cars') ?>">
                                <div class="inner-group grid">
                                    <div class="form-group">
                                        <div class="group-select">
                                            <select name="main-category" id="main-category"
                                                    class="form-control nice-select">
                                                <option value="" disabled selected>Make</option>
                                                <?php foreach ($categories1 as $category): ?>
                                                    <!-- Use categories1 here -->
                                                    <option value="<?= $category['id'] ?>">
                                                        <?= $category['category_name'] ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <div class="group-select">
                                            <select name="sub-category" id="sub-category"
                                                    class="form-control nice-select">
                                                <option value="" disabled selected>Select a Model</option>
                                            </select>
                                        </div>
                                    </div>

                                    <script>
                                        // Data from PHP
                                        const categories = <?= json_encode($categories1); ?>; // Use categories1 here

                                        document.getElementById('main-category').addEventListener('change', function () {
                                            const mainCategoryId = this.value;
                                            const subCategorySelect = document.getElementById('sub-category');

                                            // Clear existing options
                                            subCategorySelect.innerHTML = '<option value="" disabled selected>Select a Model</option>';

                                            // Find selected main category
                                            const selectedCategory = categories.find(category => category.id == mainCategoryId);

                                            // Populate subcategory dropdown
                                            if (selectedCategory && selectedCategory.sub_categories) {
                                                selectedCategory.sub_categories.forEach(subCategory => {
                                                    const option = document.createElement('option');
                                                    option.value = subCategory.id;
                                                    option.textContent = subCategory.model_name;
                                                    subCategorySelect.appendChild(option);
                                                });
                                            }
                                        });
                                    </script>

                                    <div class="form-group">
                                        <div class="widget widget-price">
                                            <div class="caption flex-two">
                                                <p class="price-range">Price</p>
                                            </div>
                                            <div id="slider-range"></div>
                                            <div class="slider-labels">
                                                <div>
                                                    <input type="hidden" name="min-value" id="min-value1">
                                                    <input type="hidden" name="max-value" id="max-value1">
                                                </div>
                                                <div class="number-range">
                                                    <span id="slider-range-value1"></span>
                                                    <span id="slider-range-value2"></span>
                                                </div>
                                            </div>
                                        </div><!-- /.widget_price -->
                                    </div>

                                    <div class="form-group">
                                        <button type="submit" class="button-search-listing">
                                            <i class="icon-search-1"></i>
                                            Search Cars
                                        </button>
                                    </div>
                                </div>
                            </form>


                        </div>
                        <div class="tab-pane fade" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                            <form method="get" id="search-forms" action="<?= base_url('/cars') ?>">
                                <div class="inner-group grid">
                                    <div class="form-group">
                                        <div class="group-select">
                                            <select name="main-category" id="main-category2"
                                                    class="form-control nice-select">
                                                <option value="" disabled selected>Make</option>
                                                <?php foreach ($categories1 as $category): ?>
                                                    <!-- Use categories1 here -->
                                                    <option value="<?= $category['id'] ?>">
                                                        <?= $category['category_name'] ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <div class="group-select">
                                            <select name="sub-category" id="sub-category2"
                                                    class="form-control nice-select">
                                                <option value="" disabled selected>Select a Model</option>
                                            </select>
                                        </div>
                                    </div>

                                    <script>
                                        // Data from PHP
                                        const categories2 = <?= json_encode($categories1); ?>; // Use categories1 here

                                        document.getElementById('main-category2').addEventListener('change', function () {
                                            const mainCategoryId2 = this.value;
                                            const subCategorySelect2 = document.getElementById('sub-category2');

                                            // Clear existing options
                                            subCategorySelect2.innerHTML = '<option value="" disabled selected>Select a Model</option>';

                                            // Find selected main category
                                            const selectedCategory2 = categories.find(category => category.id == mainCategoryId2);

                                            // Populate subcategory dropdown
                                            if (selectedCategory2 && selectedCategory2.sub_categories) {
                                                selectedCategory2.sub_categories.forEach(subCategory => {
                                                    const option = document.createElement('option');
                                                    option.value = subCategory.id;
                                                    option.textContent = subCategory.model_name;
                                                    subCategorySelect2.appendChild(option);
                                                });
                                            }
                                        });
                                    </script>

                                    <div class="form-group">
                                        <div class="widget widget-price ">
                                            <div class="caption flex-two">
                                                <p class="price-range">Price</p>
                                            </div>
                                            <div id="slider-range2"></div>
                                            <div class="slider-labels">
                                                <div>
                                                    <input type="hidden" name="min-value" id="min-value2">
                                                    <input type="hidden" name="max-value" id="max-value2">
                                                </div>
                                                <div class="number-range">
                                                    <span id="slider-range-value01"></span>
                                                    <span id="slider-range-value02"></span>
                                                </div>
                                            </div>
                                        </div><!-- /.widget_price -->
                                    </div>
                                    <input type="hidden" name="condition" value="Used">
                                    <div class="form-group">
                                        <button type="submit" class="button-search-listing">
                                            <i class="icon-search-1"></i>
                                            Search Cars
                                        </button>
                                    </div>
                                </div>
                            </form>
                        </div>
                        <div class="tab-pane fade" id="contact" role="tabpanel" aria-labelledby="contact-tab">
                            <form method="get" id="search-forms" action="<?= base_url('/cars') ?>">
                                <div class="inner-group grid">
                                    <div class="form-group">
                                        <div class="group-select">
                                            <select name="main-category" id="main-category3"
                                                    class="form-control nice-select">
                                                <option value="" disabled selected>Make</option>
                                                <?php foreach ($categories1 as $category): ?>
                                                    <!-- Use categories1 here -->
                                                    <option value="<?= $category['id'] ?>">
                                                        <?= $category['category_name'] ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                    </div>

                                    <div class="form-group">
                                        <div class="group-select">
                                            <select name="sub-category" id="sub-category3"
                                                    class="form-control nice-select">
                                                <option value="" disabled selected>Select a Model</option>
                                            </select>
                                        </div>
                                    </div>

                                    <script>
                                        // Data from PHP
                                        const categories3 = <?= json_encode($categories1); ?>; // Use categories1 here

                                        document.getElementById('main-category3').addEventListener('change', function () {
                                            console.log("adaca");
                                            const mainCategoryId3 = this.value;
                                            const subCategorySelect3 = document.getElementById('sub-category3');

                                            // Clear existing options
                                            subCategorySelect3.innerHTML = '<option value="" disabled selected>Select a Model</option>';

                                            // Find selected main category
                                            const selectedCategory3 = categories3.find(category => category.id == mainCategoryId3);

                                            // Populate subcategory dropdown
                                            if (selectedCategory3 && selectedCategory3.sub_categories) {
                                                selectedCategory3.sub_categories.forEach(subCategory3 => {
                                                    const option3 = document.createElement('option');
                                                    option3.value = subCategory3.id;
                                                    option3.textContent = subCategory3.model_name;
                                                    subCategorySelect3.appendChild(option3);
                                                });
                                            }
                                        });
                                    </script>
                                    <div class="form-group">
                                        <div class="widget widget-price ">
                                            <div class="caption flex-two">
                                                <p class="price-range">Price</p>
                                            </div>
                                            <div id="slider-range3"></div>
                                            <div class="slider-labels">
                                                <div>
                                                    <input type="hidden" name="min-value" id="min-value3">
                                                    <input type="hidden" name="max-value" id="max-value3">
                                                </div>
                                                <div class="number-range">
                                                    <span id="slider-range-value03"></span>
                                                    <span id="slider-range-value04"></span>
                                                </div>
                                            </div>
                                        </div><!-- /.widget_price -->
                                    </div>
                                    <input type="hidden" name="condition" value="New">
                                    <div class="form-group">
                                        <button type="submit" class="button-search-listing">
                                            <i class="icon-search-1"></i>
                                            Search Cars
                                        </button>
                                    </div>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- Slide  search car-->

        <!-- Icon box -->
        <div class="widget-icon-box mt--115">
            <div class="themesflat-container">
                <div class="heading-section t-al-center mb-30">
                    <span class="sub-title mb-6 wow fadeInUp">Find your vehicle by brand</span>
                    <h2 class="title wow fadeInUp">Browse by Brands</h2>
                </div>
                <div class="swiper-container-customized carousel-5">
                    <div class="swiper-wrapper">
                        <?php foreach ($categories as $category): ?>
                            <div class="col-6 col-md-4 col-lg-4 col-xl-2 swiper-slide">
                                <a href="<?= base_url('/cars') ?>?main-category=<?= urlencode($category['id']) ?>"
                                   class="icon-box border-line">
                                    <div class="image-box-wrap">
                                        <img src="<?= $category['image_url']; ?>" loading="lazy" alt="">
                                    </div>
                                    <span class="title-icon"><?= htmlspecialchars($category['category_name']) ?></span>
                                    <div class="btn-con-box">
                                        <i class="icon-arrow-right2"></i>
                                    </div>
                                </a>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
            </div>
        </div>
        <!-- Icon box -->

        <!-- Widget Tab All Car Service -->
        <div class="widget-car-service">
            <div class="themesflat-container">
                <div class="header-section tab-car-service">
                    <div class="heading-section">
                        <span class="sub-title mb-6 wow fadeInUp">Trusted Vehicle Dealer Service</span>
                        <h2 class="title wow fadeInUp">Explore Latest Vehicles</h2>
                    </div>

                </div>

                <div class="tab-content" id="pills-tabContent">

                    <div id="carouselExampleAutoplayingAll" class="carousel slide" data-bs-ride="carousel">
                        <div class="carousel-inner">
                            <?php
                            $carChunks = array_chunk($allCars, 6); // Divide the cars into chunks of 6
                            foreach ($carChunks as $index => $cars): ?>
                                <div class="carousel-item <?php if ($index === 0) echo 'active'; ?>">
                                    <div class="car-list-item p-5">
                                        <?php foreach ($cars as $car): ?>
                                            <div class="tf-car-service" data-car-id="<?= htmlspecialchars($car['id']) ?>">
                                                <?php if ($car['sale_status'] == "SOLD"): ?>
                                                    <img src="../assets/images/sold/sold.png" class="sold-tag" alt="sold tag">
                                                <?php endif; ?>
                                                <a href="<?= base_url('cars/' . $car['id']); ?>" class="image">
                                                    <!-- Display car image and details -->
                                                    <?php
                                                    if (isset($car['images'])) {
                                                        $images = json_decode($car['images'], true);
                                                        foreach ($images as $image) {
                                                            if (isset($image['image_type']) && $image['image_type'] === 'image') {
                                                                echo '<div class="listing-item active" title="' . $car['title'] . '">';
                                                                echo '<div class="images">';
                                                                echo '<img src="' . base_url('/' . $image['img_url']) . '" class="swiper-image tfcl-light-gallery car-img-resize" loading="lazy" alt="image" />';
                                                                echo '</div></div>';
                                                                break;
                                                            }
                                                        }
                                                    } else {
                                                        echo 'No images available.';
                                                    }
                                                    ?>
                                                </a>
                                                <div class="content">
                                                    <h6 class="title"><a href="<?= base_url('cars/' . $car['id']); ?>"><?= htmlspecialchars($car['title']) ?: 'No Title' ?></a></h6>
                                                    <?php if ($car['sale_status'] == "AVAILABLE"): ?>
                                                        <span class="price"><?= htmlspecialchars($car['sale_price']) ? '£' . htmlspecialchars($car['sale_price']) : 'Contact for price' ?></span>
                                                    <?php endif; ?>
                                                    <div class="description">
                                                        <ul>
                                                            <li class="listing-information fuel">
                                                                <i class="icon-gasoline-pump-1"></i>
                                                                <div class="inner">
                                                                    <span>Fuel type</span>
                                                                    <p><?= htmlspecialchars($car['fuel_type']) ?></p>
                                                                </div>
                                                            </li>
                                                            <?php if ($car['condition'] == "Used"): ?>
                                                                <li class="listing-information size-engine">
                                                                    <i class="icon-Group1"></i>
                                                                    <div class="inner">
                                                                        <span>Mileage</span>
                                                                        <p><?= esc($car['mileage']) ? esc($car['mileage']) . ' km' : 'N/A'; ?>
                                                                        </p>
                                                                    </div>
                                                                </li>
                                                            <?php else: ?>
                                                                <li class="listing-information size-engine">
                                                                    <i class="icon-Format-color-fill"></i>
                                                                    <div class="inner">
                                                                        <span>Color</span>
                                                                        <p><?= esc($car['color']) ? esc($car['color']) : 'N/A'; ?>
                                                                        </p>
                                                                    </div>
                                                                </li>
                                                            <?php endif; ?>
                                                            <li class="listing-information transmission">
                                                                <i class="icon-gearbox-1"></i>
                                                                <div class="inner">
                                                                    <span>Transmission</span>
                                                                    <p><?= htmlspecialchars($car['transmission']) ?></p>
                                                                </div>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                    <div class="bottom-btn-wrap">
                                                        <div class="btn-read-more">
                                                            <a class="more-link" href="<?= base_url('cars/' . $car['id']); ?>">
                                                                <span>View details</span>
                                                                <i class="icon-arrow-right2"></i>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        <button class="carousel-control-prev carousel-btn" type="button" data-bs-target="#carouselExampleAutoplayingAll" data-bs-slide="prev">
                            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                            <span class="visually-hidden">Previous</span>
                        </button>
                        <button class="carousel-control-next carousel-btn" type="button" data-bs-target="#carouselExampleAutoplayingAll" data-bs-slide="next">
                            <span class="carousel-control-next-icon" aria-hidden="true"></span>
                            <span class="visually-hidden">Next</span>
                        </button>
                    </div>
                </div>
            </div>
        </div>
        <!-- Widget Tab All Car Service -->

        <!-- Widget Tab Car Service -->
        <div class="widget-car-service">
            <div class="themesflat-container">
                <div class="header-section tab-car-service">
                    <div class="heading-section">
                        <span class="sub-title mb-6 wow fadeInUp">Trusted Car Dealer Service</span>
                        <h2 class="title wow fadeInUp">Explore Latest Cars</h2>
                    </div>

                </div>

                <div class="tab-content" id="pills-tabContent">

                    <div id="carouselExampleAutoplaying" class="carousel slide" data-bs-ride="carousel">
                        <div class="carousel-inner">
                            <?php
                            $carChunks = array_chunk($passengerCars, 6); // Divide the cars into chunks of 6
                            foreach ($carChunks as $index => $cars): ?>
                                <div class="carousel-item <?php if ($index === 0) echo 'active'; ?>">
                                    <div class="car-list-item p-5">
                                        <?php foreach ($cars as $car): ?>
                                            <div class="tf-car-service" data-car-id="<?= htmlspecialchars($car['id']) ?>">
                                                <?php if ($car['sale_status'] == "SOLD"): ?>
                                                    <img src="../assets/images/sold/sold.png" class="sold-tag" alt="sold tag">
                                                <?php endif; ?>
                                                <a href="<?= base_url('cars/' . $car['id']); ?>" class="image">
                                                    <!-- Display car image and details -->
                                                    <?php
                                                    if (isset($car['images'])) {
                                                        $images = json_decode($car['images'], true);
                                                        foreach ($images as $image) {
                                                            if (isset($image['image_type']) && $image['image_type'] === 'image') {
                                                                echo '<div class="listing-item active" title="' . $car['title'] . '">';
                                                                echo '<div class="images">';
                                                                echo '<img src="' . base_url('/' . $image['img_url']) . '" class="swiper-image tfcl-light-gallery car-img-resize" loading="lazy" alt="image" />';
                                                                echo '</div></div>';
                                                                break;
                                                            }
                                                        }
                                                    } else {
                                                        echo 'No images available.';
                                                    }
                                                    ?>
                                                </a>
                                                <div class="content">
                                                    <h6 class="title"><a href="<?= base_url('cars/' . $car['id']); ?>"><?= htmlspecialchars($car['title']) ?: 'No Title' ?></a></h6>
                                                    <?php if ($car['sale_status'] == "AVAILABLE"): ?>
                                                        <span class="price"><?= htmlspecialchars($car['sale_price']) ? '£' . htmlspecialchars($car['sale_price']) : 'Contact for price' ?></span>
                                                    <?php endif; ?>
                                                    <div class="description">
                                                        <ul>
                                                            <li class="listing-information fuel">
                                                                <i class="icon-gasoline-pump-1"></i>
                                                                <div class="inner">
                                                                    <span>Fuel type</span>
                                                                    <p><?= htmlspecialchars($car['fuel_type']) ?></p>
                                                                </div>
                                                            </li>
                                                            <?php if ($car['condition'] == "Used"): ?>
                                                                <li class="listing-information size-engine">
                                                                    <i class="icon-Group1"></i>
                                                                    <div class="inner">
                                                                        <span>Mileage</span>
                                                                        <p><?= esc($car['mileage']) ? esc($car['mileage']) . ' km' : 'N/A'; ?>
                                                                        </p>
                                                                    </div>
                                                                </li>
                                                            <?php else: ?>
                                                                <li class="listing-information size-engine">
                                                                    <i class="icon-Format-color-fill"></i>
                                                                    <div class="inner">
                                                                        <span>Color</span>
                                                                        <p><?= esc($car['color']) ? esc($car['color']) : 'N/A'; ?>
                                                                        </p>
                                                                    </div>
                                                                </li>
                                                            <?php endif; ?>
                                                            <li class="listing-information transmission">
                                                                <i class="icon-gearbox-1"></i>
                                                                <div class="inner">
                                                                    <span>Transmission</span>
                                                                    <p><?= htmlspecialchars($car['transmission']) ?></p>
                                                                </div>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                    <div class="bottom-btn-wrap">
                                                        <div class="btn-read-more">
                                                            <a class="more-link" href="<?= base_url('cars/' . $car['id']); ?>">
                                                                <span>View details</span>
                                                                <i class="icon-arrow-right2"></i>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        <button class="carousel-control-prev carousel-btn" type="button" data-bs-target="#carouselExampleAutoplaying" data-bs-slide="prev">
                            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                            <span class="visually-hidden">Previous</span>
                        </button>
                        <button class="carousel-control-next carousel-btn" type="button" data-bs-target="#carouselExampleAutoplaying" data-bs-slide="next">
                            <span class="carousel-control-next-icon" aria-hidden="true"></span>
                            <span class="visually-hidden">Next</span>
                        </button>
                    </div>
                </div>
            </div>
        </div>
        <!-- Widget Tab Car Service -->


        <!-- Widget Tab Truck Service -->
        <div class="widget-car-service">
            <div class="themesflat-container">
                <div class="header-section tab-car-service">
                    <div class="heading-section">
                        <span class="sub-title mb-6 wow fadeInUp">Trusted Car Dealer Service</span>
                        <h2 class="title wow fadeInUp">Explore Latest Commercial Vehicles</h2>
                    </div>

                </div>

                <div class="tab-content" id="pills-tabContent">

                    <div id="carouselExampleAutoplaying2" class="carousel slide" data-bs-ride="carousel">
                        <div class="carousel-inner">
                            <?php
                            $carChunks = array_chunk($commercialCars, 6); // Divide the cars into chunks of 6
                            foreach ($carChunks as $index => $cars): ?>
                                <div class="carousel-item <?php if ($index === 0) echo 'active'; ?>">
                                    <div class="car-list-item p-5">
                                        <?php foreach ($cars as $car): ?>
                                            <div class="tf-car-service" data-car-id="<?= htmlspecialchars($car['id']) ?>">
                                                <?php if ($car['sale_status'] == "SOLD"): ?>
                                                    <img src="../assets/images/sold/sold.png" alt="sold tag">
                                                <?php endif; ?>
                                                <a href="<?= base_url('cars/' . $car['id']); ?>" class="image">
                                                    <!-- Display car image and details -->
                                                    <?php
                                                    if (isset($car['images'])) {
                                                        $images = json_decode($car['images'], true);
                                                        foreach ($images as $image) {
                                                            if (isset($image['image_type']) && $image['image_type'] === 'image') {
                                                                echo '<div class="listing-item active" title="' . $car['title'] . '">';
                                                                echo '<div class="images">';
                                                                echo '<img src="' . base_url('/' . $image['img_url']) . '" class="swiper-image tfcl-light-gallery car-img-resize" loading="lazy" alt="image" />';
                                                                echo '</div></div>';
                                                                break;
                                                            }
                                                        }
                                                    } else {
                                                        echo 'No images available.';
                                                    }
                                                    ?>
                                                </a>
                                                <div class="content">
                                                    <h6 class="title"><a href="<?= base_url('cars/' . $car['id']); ?>"><?= htmlspecialchars($car['title']) ?: 'No Title' ?></a></h6>
                                                    <?php if ($car['sale_status'] == "AVAILABLE"): ?>
                                                        <span class="price"><?= htmlspecialchars($car['sale_price']) ? '£' . htmlspecialchars($car['sale_price']) : 'Contact for price' ?></span>
                                                    <?php endif; ?>
                                                    <div class="description">
                                                        <ul>
                                                            <li class="listing-information fuel">
                                                                <i class="icon-gasoline-pump-1"></i>
                                                                <div class="inner">
                                                                    <span>Fuel type</span>
                                                                    <p><?= htmlspecialchars($car['fuel_type']) ?></p>
                                                                </div>
                                                            </li>
                                                            <?php if ($car['condition'] == "Used"): ?>
                                                                <li class="listing-information size-engine">
                                                                    <i class="icon-Group1"></i>
                                                                    <div class="inner">
                                                                        <span>Mileage</span>
                                                                        <p><?= esc($car['mileage']) ? esc($car['mileage']) . ' km' : 'N/A'; ?>
                                                                        </p>
                                                                    </div>
                                                                </li>
                                                            <?php else: ?>
                                                                <li class="listing-information size-engine">
                                                                    <i class="icon-Format-color-fill"></i>
                                                                    <div class="inner">
                                                                        <span>Color</span>
                                                                        <p><?= esc($car['color']) ? esc($car['color']) : 'N/A'; ?>
                                                                        </p>
                                                                    </div>
                                                                </li>
                                                            <?php endif; ?>
                                                            <li class="listing-information transmission">
                                                                <i class="icon-gearbox-1"></i>
                                                                <div class="inner">
                                                                    <span>Transmission</span>
                                                                    <p><?= htmlspecialchars($car['transmission']) ?></p>
                                                                </div>
                                                            </li>
                                                        </ul>
                                                    </div>
                                                    <div class="bottom-btn-wrap">
                                                        <div class="btn-read-more">
                                                            <a class="more-link" href="<?= base_url('cars/' . $car['id']); ?>">
                                                                <span>View details</span>
                                                                <i class="icon-arrow-right2"></i>
                                                            </a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
                        <button class="carousel-control-prev carousel-btn" type="button" data-bs-target="#carouselExampleAutoplaying2" data-bs-slide="prev">
                            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                            <span class="visually-hidden">Previous</span>
                        </button>
                        <button class="carousel-control-next carousel-btn" type="button" data-bs-target="#carouselExampleAutoplaying2" data-bs-slide="next">
                            <span class="carousel-control-next-icon" aria-hidden="true"></span>
                            <span class="visually-hidden">Next</span>
                        </button>
                    </div>
                </div>
            </div>
        </div>
        <!-- Widget Tab Truck Service -->

        <!-- Widget Testimonial car -->
        <div class="widget-testimonial-car">
            <div class="themesflat-container">
                <div class="heading-section t-al-center mb-60">
                    <span class="sub-title mb-6 wow fadeInUp">Testimonials</span>
                    <h2 class="title wow fadeInUp">What Client's Says</h2>
                </div>
                <div class="testimonial-car-v1 pb-5">
                    <div class="swiper testimonial-swiper">
                        <div class="swiper-wrapper">
                            <?php foreach ($testimonials as $testimonial): ?>
                            <div class="testimonial-content swiper-slide flex">
                                <div class="image-thumbs col-4">
                                    <div class="icon-quote">
                                        <i class="icon-581419696"></i>
                                    </div>
                                    <img src="<?= $testimonial['img_url'] ?>" loading="lazy" alt="image">
                                </div>
                                <div class="testimonial-text-content col-8 ">
                                    <span class="sub-title">R&R Auto World</span>
                                    <h3 class="title"><?= $testimonial['title'] ?></h3>
                                    <p class="description">"<?= $testimonial['description'] ?>"</p>
                                    <div class="wrap-avatar">
                                        <div class="info">
                                            <div class="name"><?= $testimonial['customer_name'] ?></div>
                                            <div class="position"><?= $testimonial['customer_address'] ?></div>
                                        </div>
                                    </div>
                                </div>

                            </div>
                            <?php endforeach; ?>
                        </div>
                        <div class="btn-next-prev">
                            <div class="swiper-button-next-tes next">
                                <i class="icon-Vector-11"></i>
                            </div>
                            <div class="swiper-button-prev-tes prev">
                                <i class="icon-Vector-2"></i>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- Widget Testimonial car -->

    </div>


    <!-- Footer -->
    <footer id="footer" class="clearfix bg-footer2 pd-t81 re-hi">
        <div class="themesflat-container">
            <div class="row footer-top">
                <div class="col-lg-6 col-md-12 col-12 pd-r80 ">
                    <h2 class="title-footer-top">Do you Have <span class="red-title">something</span> to sell
                        through us?</h2>
                </div>
            </div>
            <div class="row footer-main">
                <div class="col-lg-4 col-md-6 col-12">
                    <div class="widget widget-info">

                        <svg id="Group_133" data-name="Group 133" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="250" height="auto" viewBox="0 0 742.172 248.684">
                            <defs>
                                <clipPath id="clip-path">
                                    <rect id="Rectangle_171" data-name="Rectangle 171" width="742.172" height="248.684" fill="none"/>
                                </clipPath>
                            </defs>
                            <g id="Group_132" data-name="Group 132" clip-path="url(#clip-path)">
                                <path id="Path_430" data-name="Path 430" d="M248.684,124.342A124.342,124.342,0,1,1,124.342,0,124.342,124.342,0,0,1,248.684,124.342" fill="#54080b"/>
                                <path id="Path_431" data-name="Path 431" d="M85.859,66.105c4.62-5.953,10.033-11.173,17.347-13.482a70.772,70.772,0,0,1,14.9-2.79,151.532,151.532,0,0,1,34.061,1.037c11.955,1.63,23.211,5.672,34.235,10.352,14.841,6.3,17.133,25.47,4.324,35.23a61.485,61.485,0,0,1-17.606,8.989c-6.314,2.166-12.627,4.338-18.895,6.633-1.985.728-2.082,1.565-.694,3.127q6.675,7.512,13.389,14.987c10.968,12.2,22.15,24.222,32.867,36.642,10.285,11.918,15.96,25.757,16.449,41.271A124.3,124.3,0,1,0,7.559,167.133q39.129-50.53,78.3-101.028" fill="#d7001c"/>
                                <path id="Path_432" data-name="Path 432" d="M193.906,215.356c1.058-14.56-4.147-26.705-13.741-37.288q-19.368-21.363-38.709-42.749a104.735,104.735,0,0,1-8.1-9.71c-8.054-11.466-5.524-23.5,6.292-31.04,6.053-3.861,12.857-5.869,19.583-8.1,3.864-1.279,7.7-2.661,11.5-4.11a1.7,1.7,0,0,0,.162-.089c1.585-1.859-1.057-3.567-1.057-3.567l0,0a98.752,98.752,0,0,0-34.206-6.88c-.435-.01-.87-.035-1.3-.058v0l-7.94.146c-3.47.162-6.943.3-10.41.493a13.575,13.575,0,0,0-10.306,5.067Q97,88.291,88.5,99.242q-31.6,40.71-63.145,81.464-3.439,4.442-6.871,8.889a124.367,124.367,0,0,0,174.578,38.379q.393-6.311.841-12.618" fill="#d7001c"/>
                                <path id="Path_433" data-name="Path 433" d="M167.06,115.46q-6.717-7.473-13.389-14.986c-1.388-1.563-1.291-2.4.694-3.127,6.268-2.3,12.581-4.468,18.895-6.634a61.484,61.484,0,0,0,17.606-8.989c12.809-9.76,10.517-28.93-4.324-35.23-11.024-4.68-22.28-8.722-34.235-10.351A151.532,151.532,0,0,0,118.246,35.1a70.772,70.772,0,0,0-14.9,2.79C96.028,40.205,90.615,45.424,86,51.377Q45.109,104.064,4.289,156.8a123.571,123.571,0,0,0,9.443,24.391q5.883-7.606,11.761-15.217,31.542-40.755,63.145-81.463,8.5-10.95,17.171-21.767a13.581,13.581,0,0,1,10.306-5.067c3.467-.195,6.94-.33,10.41-.493l7.94-.145v0c.435.023.87.048,1.305.058a98.761,98.761,0,0,1,34.207,6.879l0,0s2.642,1.707,1.057,3.566a1.514,1.514,0,0,1-.162.09c-3.8,1.448-7.636,2.83-11.5,4.109-6.726,2.227-13.53,4.235-19.583,8.1-11.816,7.536-14.346,19.574-6.292,31.04a104.59,104.59,0,0,0,8.1,9.709Q160.9,142.009,180.3,163.34c9.6,10.583,14.8,22.728,13.741,37.288-.674,9.277-1.216,18.564-1.745,27.852a124.95,124.95,0,0,0,23.643-20.057c.189-3.593.35-7.186.455-10.781.505-17.257-5.247-32.538-16.467-45.54-10.717-12.419-21.9-24.438-32.867-36.642" fill="#fff"/>
                                <path id="Path_434" data-name="Path 434" d="M351.26,47.808q28.5,0,40.47,7.028t11.968,25.5a25.024,25.024,0,0,1-6.929,17.683q-7.716,8.208-21.731,9.63,13.542,0,20.157,5.512t6.613,15.589v23.621H368.9V132.055q0-6.768-5.669-9.369t-16.849-2.6H305.437v32.281H273.628V47.808Zm-4.881,49.761q13.541,0,18.66-2.613t5.118-10.693q0-7.444-2.992-9.822-4.41-3.485-18.9-3.484H305.437V97.569Z" fill="#fff"/>
                                <path id="Path_435" data-name="Path 435" d="M555.182,126.544l23.149,13.07-16.377,13.07-18.74-10.393q-6.93,6.613-20.628,9.133-9.607,1.731-25.668,1.732H479.282q-23.148,0-35.116-5.2Q425.9,139.929,425.9,118.67q0-11.965,5.354-17.794,6.141-6.613,21.889-8.661a41.187,41.187,0,0,1-7.244-7.4,19.12,19.12,0,0,1-3.149-10.866q0-15.43,14.172-21.1Q467.628,48.6,493.139,48.6q27.241,0,37.636,4.409,13.7,5.827,13.7,22.991v6.771H515.5q0-11.179-2.678-13.543-3.467-3.147-18.581-3.149-11.183,0-14.33.787-6.459,1.577-6.456,7.244,0,5.042,2.2,7.559,1.731,1.889,8.818,5.826l45.509,25.353a16.058,16.058,0,0,0,1.26-6.3,13.632,13.632,0,0,0-1.26-6.142h27.4q.787,4.412,1.024,7.244t.236,4.567a28.72,28.72,0,0,1-1.417,9.133q-.473,1.417-2.048,5.2m-58.578,8.5a101.838,101.838,0,0,0,14.645-.787,24.388,24.388,0,0,0,9.92-4.094L472.2,103.4q-10.08,1.575-12.834,4.172t-2.756,9.685q0,11.811,7.716,15.589,4.566,2.208,17.165,2.205Z" fill="#fff"/>
                                <path id="Path_436" data-name="Path 436" d="M671.867,47.808q28.5,0,40.47,7.028t11.967,25.5a25.023,25.023,0,0,1-6.928,17.683q-7.717,8.208-21.731,9.63,13.542,0,20.156,5.512t6.614,15.589v23.621H689.5V132.055q0-6.768-5.669-9.369t-16.85-2.6H626.043v32.281H594.234V47.808Zm-4.882,49.761q13.541,0,18.66-2.613t5.118-10.693q0-7.444-2.992-9.822-4.41-3.485-18.9-3.484H626.043V97.569Z" fill="#fff"/>
                                <path id="Path_437" data-name="Path 437" d="M303.64,168.488l16.654,32.145H310.079l-3.1-5.954H286.745l-2.953,5.954H273.286l16.363-32.145Zm-13.8,19.268h14.088l-7.117-13.41Z" fill="#d7001b"/>
                                <path id="Path_438" data-name="Path 438" d="M365.364,168.585v21.2a8.891,8.891,0,0,1-4.962,8.448q-4.964,2.64-15.08,2.639-10.893,0-15.347-2.131-5.471-2.614-5.47-9.73v-20.43h10.021v20.43q0,3.05,2.082,4.018,1.742.823,6.68.823h3.2a28.6,28.6,0,0,0,6.584-.436q2.274-.726,2.275-3.631v-21.2Z" fill="#d7001b"/>
                                <path id="Path_439" data-name="Path 439" d="M407.868,168.488v7.165H393.684v24.98H383.565v-24.98H369.623v-7.165Z" fill="#d7001b"/>
                                <path id="Path_440" data-name="Path 440" d="M453.036,178.185V190.6q0,4.945-4.938,7.757a19.872,19.872,0,0,1-9.924,2.376H434.2q-5.375,0-7.044-.016t-2.929-.081a16.073,16.073,0,0,1-9.295-2.614,8.6,8.6,0,0,1-4.163-7.455V178.17q0-4.21,4.357-7.116a15.19,15.19,0,0,1,8.569-2.566h14.91a18.971,18.971,0,0,1,9.876,2.424q4.55,2.717,4.551,7.273m-10.167,9.394v-5.844a6.977,6.977,0,0,0-1.694-5.07q-1.889-1.98-5.954-1.98h-8.037a6.087,6.087,0,0,0-4.817,1.787,7.349,7.349,0,0,0-1.573,5.022v6.085a7.076,7.076,0,0,0,1.549,4.877,6.166,6.166,0,0,0,4.841,1.739h8.037q4.114,0,5.881-1.642a6.472,6.472,0,0,0,1.767-4.974" fill="#d7001b"/>
                                <path id="Path_441" data-name="Path 441" d="M532.091,168.488H542.4l-13.362,32.145h-11.86L507.5,177.978l-7.5,22.656H488.327l-12.973-32.145h10.214l8.617,23.431,7.939-23.431H513.4l9.828,23.431Z" fill="#d7001b"/>
                                <path id="Path_442" data-name="Path 442" d="M586.7,178.185V190.6q0,4.945-4.938,7.757a19.872,19.872,0,0,1-9.924,2.376h-3.97q-5.375,0-7.044-.016t-2.929-.081a16.073,16.073,0,0,1-9.295-2.614,8.6,8.6,0,0,1-4.163-7.455V178.17q0-4.21,4.357-7.116a15.19,15.19,0,0,1,8.569-2.566h14.91a18.971,18.971,0,0,1,9.876,2.424q4.55,2.717,4.551,7.273m-10.167,9.394v-5.844a6.977,6.977,0,0,0-1.694-5.07q-1.889-1.98-5.954-1.98h-8.037a6.087,6.087,0,0,0-4.817,1.787,7.349,7.349,0,0,0-1.573,5.022v6.085a7.076,7.076,0,0,0,1.549,4.877,6.166,6.166,0,0,0,4.841,1.739h8.037q4.115,0,5.881-1.642a6.472,6.472,0,0,0,1.767-4.974" fill="#d7001b"/>
                                <path id="Path_443" data-name="Path 443" d="M616.907,168.488q8.762,0,12.441,2.16t3.68,7.839a7.7,7.7,0,0,1-2.13,5.437,10.222,10.222,0,0,1-6.681,2.961q4.163,0,6.2,1.694a5.893,5.893,0,0,1,2.033,4.792v7.262H622.329v-6.245a2.9,2.9,0,0,0-1.743-2.88,12.949,12.949,0,0,0-5.18-.8H602.819v9.924H593.04V168.488Zm-1.5,15.3q4.163,0,5.737-.8t1.574-3.288q0-2.288-.92-3.019-1.357-1.071-5.81-1.071H602.819v8.181Z" fill="#d7001b"/>
                                <path id="Path_444" data-name="Path 444" d="M672.772,193.469v7.165H640.385V168.489H650.5v24.98Z" fill="#d7001b"/>
                                <path id="Path_445" data-name="Path 445" d="M720.313,180.415l.1,5.382q0,7.128-2.9,10.424-3.873,4.412-14.232,4.412H677.566V168.488h24.108q9.681,0,14.16,2.813t4.479,9.114m-20.091,13.828q6.293,0,8.375-2.33,1.5-1.7,1.5-6.164v-4.8a4.452,4.452,0,0,0-1.743-3.932q-2.131-1.5-7.552-1.5H687.538v18.735Z" fill="#d7001b"/>
                                <path id="Path_446" data-name="Path 446" d="M730.878,38.486v1.021h-2.62V48.1H727V39.507h-2.634V38.486Z" fill="#fff"/>
                                <path id="Path_447" data-name="Path 447" d="M742.172,38.555V48.1h-1.255V40.982L737.745,48.1h-.883l-3.185-7.129V48.1h-1.255V38.555h1.352l3.53,7.888,3.53-7.888Z" fill="#fff"/>
                            </g>
                        </svg>
                        <p>R&R Autoworld offering a wide selection of new and pre-owned vehicles with a focus on customer satisfaction and hassle-free service.</p>
                    </div>
                </div>

                <div class="col-lg-4 col-md-12 col-12">
                    <div class="widget widget-info">
                        <ul>
                            <li>
                                <i class="icon-Vector1"></i>
                                <p>4 Cuthbert court, 33 falcon way, South Ockendon, RM155LQ</p>
                            </li>
                        </ul>

                    </div>

                </div>
                <div class="col-lg-4 col-md-12 col-12">
                    <div class="widget widget-info">
                        <ul>
                            <li>
                                <i class="icon-Group-1"></i>
                                <p style="text-transform: none;">info@randrautoworld.co.uk</p>
                            </li>
                            <li>
                                <i class="icon-Group-1"></i>
                                <p style="text-transform: none;">ruchini.shell@gmail.com</p>
                            </li>
                        </ul>
                    </div>

                </div>


            </div>
            <div class="row footer-bottom">
                <div class="col-12 col-md-6 col-lg-6 col-xl-4 col-xxl-4">

                </div>
                <div class="col-12 col-md-6 col-lg-6 col-xl-4 col-xxl-4">
                    <ul class="social-icon">
                        <li>
                            <a href="#"><i class="icon-6"></i></a>
                        </li>
                        <li>
                            <a href="#"><i class="icon-4"></i></a>
                        </li>
                        <li>
                            <a href="#"><i class="icon-5"></i></a>
                        </li>
                        <li>
                            <a href="#"><i class="icon-7"></i></a>
                        </li>
                    </ul>
                </div>
                <div class="col-md-12 col-lg-12 col-xl-4 col-xxl-4">
                    <!--                        <ul class="bottom-bar-menu">-->
                    <!--                            <li><a href="#">Privacy & Policy</a></li>-->
                    <!--                            <li><a href="#">Terms & Conditions</a></li>-->
                    <!--                        </ul>-->
                </div>
            </div>
        </div>
        <img src="<?= base_url('assets/images/page/ft-left.png') ?>" loading="lazy" alt="left" class="shape-left">
        <img src="<?= base_url('assets/images/page/ft-right.png') ?>" loading="lazy" alt="right" class="shape-right">

        <!-- Whatsapp -->
        <a href="https://api.whatsapp.com/send?phone=+447877354243&text=Hi RNR Auto World!" class="wa-float" target="_blank">
            <i class="fa fa-whatsapp wa-wp-button"></i>
        </a>
    </footer><!-- /#footer -->

</div>
<!-- /#page -->

<!-- Button Top -->
<a id="scroll-top" class="button-go"></a>
<!-- Button Top -->



<!-- Javascript -->
<script src="<?= base_url('assets/js/jquery.min.js') ?>"></script>
<script src="<?= base_url('assets/js/bootstrap.min.js') ?>"></script>
<script src="<?= base_url('assets/js/jquery.nice-select.min.js') ?>"></script>
<script src="<?= base_url('assets/js/jquery.nice-select.min.js') ?>"></script>
<script src="<?= base_url('assets/js/swiper-bundle.min.js') ?>"></script>
<script src="<?= base_url('assets/js/swiper.js') ?>"></script>
<script src="<?= base_url('assets/js/jquery-countTo.js') ?>"></script>
<script src="<?= base_url('assets/js/nouislider.min.js') ?>"></script>
<script src="<?= base_url('assets/js/price-ranger.js') ?>"></script>
<script src="<?= base_url('assets/js/magnific-popup.min.js') ?>"></script>
<script src="<?= base_url('assets/js/wow.min.js') ?>"></script>
<script src="<?= base_url('assets/js/main.js') ?>"></script>

</body>

</html>